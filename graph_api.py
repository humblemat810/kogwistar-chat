...
class GraphAPI:
    def __init__(self, base_url: str = SERVER_URL):
        ...
        # Long timeout to avoid drops when debugging backend with breakpoints
        self.client = httpx.AsyncClient(base_url=self.base_url, timeout=3000.0)

    async def get_me(self, token: str) -> dict:
        """Gets user info from /api/auth/me using the token."""
    ...
    async def create_conversation(self, token: str, user_id: str) -> str:
        ...
        if response.status_code != 200:
            logging.error(f"create_conversation failed: {response.status_code} — {response.text}")
        response.raise_for_status()
    ...
    async def submit_turn(self, token: str, conversation_id: str, text: str) -> str:
        ...
        if response.status_code != 202:
            logging.error(f"submit_turn failed: {response.status_code} — {response.text}")
        response.raise_for_status()
